...
const EditSubrecordPage: React.FC = () => {
  const { id } = useParams();
  ...
  useEffect(() => {
    ...
    user.getIdToken().then(async (token: string) => {
      try {
        const srRes = await axios.get(`/api/subrecords/${id}`, {
        ...
        if (sr.data) {
          ...
          Object.keys(sr.data).forEach((key) => {
            const fieldObj = sr.data[key];
            vals[key] =
        ...
        if (sr.subform) {
          const formRes = await axios.get(`/api/subforms/${sr.subform}`, {
            headers: { Authorization: `Bearer ${token}` },
          });
          setSubform(formRes.data);
        }
        setLoading(false);
      } catch (err: any) {
        setError(
          err.response?.data?.error || "Erro ao buscar subregistro/subform"
        );
        setLoading(false);
      }
    });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!user || !subform || !subrecord) return;
    setSubmitting(true);
    try {
      ...
      for (const field of subform.fields) {
        ...
        if (
          ...
          const storageRef = ref(
            storage,
            `subrecords/${subform.id || subrecord.subform}/${
              field.name
      ...
      const payload = {
        data: dataToSend,
        createdBy: subrecord?.createdBy || user.email,
        org: subform?.org,
        updatedAt: formatDate(new Date()),
      };
      await axios.put(`/api/subrecords/${id}`, payload, {
        headers: { Authorization: `Bearer ${token}` },
      });
      // Após editar o subrecord, atualiza o contador no record
      try {
        const recordId = subrecord?.record;
        const subformId = subrecord?.subform;
        if (recordId && subformId) {
        ...
        // Silently ignore counter update errors to not block edit UX
      }
      navigate(`/subrecords/${id}`);
    } catch (err: any) {
      setError(err.response?.data?.error || "Erro ao atualizar subregistro");
    }
    setSubmitting(false);
  };
    ...
    <div className="container mt-4">
      <div className="card">
        <div className="card-body">
          <h2>Editar Subregistro: {subform.name}</h2>
          ...
          <button
            ...
            onClick={() => navigate(`/subrecords/${id}`)}
          >
            Voltar
          </button>
        </div>
...
export default EditSubrecordPage;
